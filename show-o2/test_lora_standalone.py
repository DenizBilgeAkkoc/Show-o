...
# coding=utf-8
"""
Standalone test for LoRA implementation - tests core LoRA functionality
...
# Add the show-o2/models directory to path to import lora directly
sys.path.insert(0, '/home/user/Show-o/show-o2/models')

# Import LoRA components directly from the lora module file (not through __init__)
from lora import (
    LoRAConfig,
    LoRALinear,
    inject_lora,
...
def test_lora_injection():
    ...
    for name, module in model.named_modules():
        if any(t in name for t in ["q_proj", "k_proj", "v_proj", "o_proj"]):
            # Only check direct module matches, not nested ones like "q_proj.original_layer"
            if "self_attn" in name and "original_layer" not in name and "lora_" not in name:
...
def test_with_qwen2_like_structure():
    """Test LoRA integration with Qwen2-like model structure."""
    print("=" * 60)
    print("Testing with Qwen2-like model structure...")
    print("=" * 60)
    ...
    # Test that forward pass works through the model
    embed = model.showo.model.embed_tokens(torch.randint(0, 1000, (2, 10)))
    for layer in model.showo.model.layers:
        # Pass through attention (now with LoRA)
        attn = layer.self_attn
        q = attn.q_proj(embed)
        k = attn.k_proj(embed)
        v = attn.v_proj(embed)
        # Simple attention-like operation
        out = attn.o_proj(q + k + v)
    print("  [PASS] Forward pass through LoRA layers works")

    print("  All Qwen2 structure tests passed!\n")
    return True
...
def test_gradient_flow():
    ...
    for name, module in model.named_modules():
        if isinstance(module, LoRALinear):
            assert module.lora_A.weight.grad is not None
            assert module.lora_B.weight.grad is not None
            print(f"  [PASS] {name} has gradients")

    # Check that original layer weights don't have gradients (frozen)
    for name, module in model.named_modules():
        if isinstance(module, LoRALinear):
            assert module.original_layer.weight.grad is None
            print(f"  [PASS] {name}.original_layer is frozen (no grad)")
...
def main():
    """Run all tests."""
    print("\n" + "=" * 60)
    print("Running LoRA Implementation Tests (Standalone)")
    print("=" * 60 + "\n")
    ...
    tests = [
        ...
        ("Qwen2 Structure", test_with_qwen2_like_structure),
